...
function Analytics() {
  ...
  const getFilteredTrades = () => {
    const tf = TIMEFRAME_OPTIONS.find(t => t.label === timeframe)
    if (!tf || tf.days === null) return allTrades
    const cutoff = new Date()
    cutoff.setDate(cutoff.getDate() - tf.days)
    const cutoffStr = cutoff.toISOString().split('T')[0]
    return allTrades.filter(t => t.date >= cutoffStr)
  }
  ...
  const worstTrade = trades.length ? trades.reduce((worst, t) => t.pnl_r < worst.pnl_r ? t : worst, trades[0]) : null
  const expectancy = totalTrades ? ((winRate / 100) * parseFloat(avgWin) + (1 - winRate / 100) * parseFloat(avgLoss)).toFixed(2) : '0'
  ...
  return (
    <div>
      <div style={{ background: '#EDE4D3', borderBottom: '1px solid #C8B89A', padding: '14px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ fontFamily: 'Lora, serif', fontSize: '16px', fontWeight: 600, color: '#2B2318' }}>Analytics</div>
          <div style={{ fontSize: '11px', color: '#9C856A', marginTop: '2px' }}>{totalTrades} trades · {timeframe === 'All' ? 'all time' : 'last ' + timeframe}</div>
        </div>
        <div style={{ display: 'flex', background: '#F5EFE4', border: '1px solid #C8B89A', borderRadius: '8px', padding: '3px', gap: '2px' }}>
          {TIMEFRAME_OPTIONS.map(tf => (
            <div key={tf.label} onClick={() => setTimeframe(tf.label)} style={{ padding: '5px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', background: timeframe === tf.label ? '#C8903A' : 'transparent', color: timeframe === tf.label ? 'white' : '#9C856A', transition: 'all 0.15s' }}>
              {tf.label}
            </div>
          ))}
        </div>
          ...
          <>
              ...
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
                  {[
                    ...
                    { label: 'Best Trade', value: bestTrade ? '+' + bestTrade.pnl_r + 'R' : '—', sub: bestTrade?.pair, color: '#3D7A52' },
                    { label: 'Worst Trade', value: worstTrade ? worstTrade.pnl_r + 'R' : '—', sub: worstTrade?.pair, color: '#9B3A28' },
                  ].map(s => (
                    <div key={s.label} style={{ background: '#EDE4D3', border: '1px solid #C8B89A', borderRadius: '10px', padding: '14px 16px' }}>
                      <div style={{ fontSize: '10px', color: '#9C856A', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '6px' }}>{s.label}</div>
            ...
            {activeTab === 'days' && (
              <div style={card}>
                ...
                {dayStats.length === 0 ? <div style={{ fontSize: '13px', color: '#9C856A' }}>No trades in this period.</div> : dayStats.sort((a, b) => b.pnl - a.pnl).map((d, i) => (
            ...
            {activeTab === 'time' && (
              <div style={card}>
                ...
                {timeStats.length === 0 ? (
                  <div style={{ fontSize: '13px', color: '#9C856A' }}>No trades with times logged in this period.</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  ...
                  <div style={{ background: '#EDE4D3', border: '1px solid #C8B89A', borderRadius: '10px', padding: '14px 16px' }}>
                    ...
                    <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '28px', fontWeight: 700, color: '#3D7A52' }}>+{Math.max(...curve.map(p => p.pnl), 0).toFixed(1)}R</div>
            ...
            {activeTab === 'tags' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                {renderTagStats(setupStats, 'By Setup', 'No setup tags in this period')}
                {renderTagStats(emotionStats, 'By Emotion', 'No emotion tags in this period')}
                {renderTagStats(mistakeStats, 'By Mistake', 'No mistake tags in this period')}
              </div>
            )}
          </>
        )}
